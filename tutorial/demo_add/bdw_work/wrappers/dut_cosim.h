...
#endif
{
    ...
    void link_signals(const char *module_path, const char *sim_domain)
    {
		...
		int din_1_vld_is_clock = esc_link_clockgen( &din_1_vld, sc_time( 0, SC_NS ), module_path, sim_domain, "din_1_vld" );
		int din_2_vld_is_clock = esc_link_clockgen( &din_2_vld, sc_time( 0, SC_NS ), module_path, sim_domain, "din_2_vld" );
		...
		esc_link_signals( &din_1_busy, module_path, sim_domain, true );
		esc_link_signals( &din_1_vld, module_path, sim_domain, true, inputDelay );
		esc_link_signals( &din_1_data, module_path, sim_domain, true, inputDelay );
		esc_link_signals( &din_2_busy, module_path, sim_domain, true );
		esc_link_signals( &din_2_vld, module_path, sim_domain, true, inputDelay );
		esc_link_signals( &din_2_data, module_path, sim_domain, true, inputDelay );
		...
		if ( din_1_vld_is_clock )
		{
			sc_signal_in_if< bool > *clock_inif = din_1_vld[0];
			sc_clock *clock_module = dynamic_cast<sc_clock*>(clock_inif);
			esc_hub_register_clock( clock_module, esc_alledge, 0, module_path, sim_domain, din_1_vld.basename() );
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "dut", "vld", scName ) ) {
			esc_report_error( esc_error, "The port 'vld' on module 'dut'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( din_2_vld_is_clock )
		{
			sc_signal_in_if< bool > *clock_inif = din_2_vld[0];
			sc_clock *clock_module = dynamic_cast<sc_clock*>(clock_inif);
			esc_hub_register_clock( clock_module, esc_alledge, 0, module_path, sim_domain, din_2_vld.basename() );
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "dut", "vld", scName ) ) {
			esc_report_error( esc_error, "The port 'vld' on module 'dut'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
