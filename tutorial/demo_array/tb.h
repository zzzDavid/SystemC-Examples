...
SC_MODULE( tb )
{
    ...
    cynw_p2p < input_t >::out dout_1; // data going in to the design
    cynw_p2p < input_t >::out dout_2; // data going in to the design
	...
	// drive the reset signal, no reset_signal_is() is needed
        ...
        din.clk_rst(clk, rst);
        dout_1.clk_rst(clk, rst);
        dout_2.clk_rst(clk, rst);
    }
