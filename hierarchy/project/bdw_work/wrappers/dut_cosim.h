...
#if defined(XM_SYSTEMC) || defined(NC_SYSTEMC)
struct dut_cosim : public xmsc_foreign_module
#else
SC_MODULE(dut_cosim)
#endif
{
    ...
    void link_signals(const char *module_path, const char *sim_domain)
    {
		...
		int find_max_x_out_busy_is_clock = esc_link_clockgen( &find_max_x_out_busy, sc_time( 0, SC_NS ), module_path, sim_domain, "find_max_x_out_busy" );
		int find_max_return_in_vld_is_clock = esc_link_clockgen( &find_max_return_in_vld, sc_time( 0, SC_NS ), module_path, sim_domain, "find_max_return_in_vld" );

		esc_link_signals( &rst, module_path, sim_domain, true, inputDelay );
		esc_link_signals( &finish, module_path, sim_domain, true );
		esc_link_signals( &find_max_x_out_busy, module_path, sim_domain, true, inputDelay );
		esc_link_signals( &find_max_x_out_vld, module_path, sim_domain, true );
		esc_link_signals( &find_max_x_out_data, module_path, sim_domain, true );
		esc_link_signals( &find_max_return_in_busy, module_path, sim_domain, true );
		esc_link_signals( &find_max_return_in_vld, module_path, sim_domain, true, inputDelay );
		esc_link_signals( &find_max_return_in_data, module_path, sim_domain, true, inputDelay );
		...
		if ( find_max_x_out_busy_is_clock )
		{
			sc_signal_in_if< bool > *clock_inif = find_max_x_out_busy[0];
			sc_clock *clock_module = dynamic_cast<sc_clock*>(clock_inif);
			esc_hub_register_clock( clock_module, esc_alledge, 0, module_path, sim_domain, find_max_x_out_busy.basename() );
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "dut", "busy", scName ) ) {
			esc_report_error( esc_error, "The port 'busy' on module 'dut'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( find_max_return_in_vld_is_clock )
		{
			sc_signal_in_if< bool > *clock_inif = find_max_return_in_vld[0];
			sc_clock *clock_module = dynamic_cast<sc_clock*>(clock_inif);
			esc_hub_register_clock( clock_module, esc_alledge, 0, module_path, sim_domain, find_max_return_in_vld.basename() );
			++registeredClocks;
